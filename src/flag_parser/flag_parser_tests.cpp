...
TEST(ParseFlags, DefaultCSV) {
  FlagOptions flags;

  ASSERT_TRUE(parse_flags({"file"}, flags));
  ASSERT_FALSE(flags.csv);
}
...
TEST(ParseFlags, CSV) {
  ...
  ASSERT_TRUE(parse_flags({"file", "--csv"}, flags));
  ASSERT_TRUE(flags.csv);
}


TEST(ParseFlags, CSVShort) {
  FlagOptions flags;

  ASSERT_TRUE(parse_flags({"file", "-c"}, flags));
  ASSERT_TRUE(flags.csv);
}

TEST(ParseFlags, CSVAndVerbose) {
    FlagOptions flags;

    ASSERT_TRUE(parse_flags({"file", "-cv"}, flags));
    ASSERT_TRUE(flags.csv);
    ASSERT_FALSE(flags.verbose);
}
